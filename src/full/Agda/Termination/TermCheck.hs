...
termDecl' = \case
    ...
    A.RecDef _ x _ _ _ _ _ _ ds -> termMutual [x] <> foldMap termDecl' ds
  ...
  where
    ...
    getName (A.RecDef _ x _ _ _ _ _ _ ds) = x : getNames ds
...
-- | Precompute the argument masks for a @(name, [projection])@ pair.  Follows
--   each projection in @path@ into the projected field type, then classifies the
--   remaining Π-arguments.

argumentMasksForPath :: Type -> [QName] -> TCM (ListInf.ListInf Bool)
argumentMasksForPath t [] = do
...
argumentMasksForPath t (d:ds) = do
  t' <- reduce t
  getDefType d t' >>= \case
    Just projTy -> reduce projTy >>= \case
      El _ (Pi dom b) -> underAbstraction dom b $ \ft ->
        -- Projection patterns occupy one slot in the clause pattern list, but
        -- should always be considered unmasked. Hence prepending @False@:
        (False :<) <$> argumentMasksForPath ft ds
      _               -> return (ListInf.pad [] True)
    Nothing           -> return (ListInf.pad [] True)
