...
data Expr
  ...
  -- Wrappers generated by scope checking to remember information that
  -- does not essentially affect type checking:
...
instance SubstExpr Expr where
  substExpr s e = case e of
    ...
    -- At the moment we only generate this for @do@, so it can't appear
    -- in a pattern synonym right-hand side, but this is future-proof.
    Qualified q e   -> Qualified q (substExpr s e)
